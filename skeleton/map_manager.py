...
if __name__=="__main__":
    ...
    currentEdgeMap={}
    for i in range(len(network.edgeid_list)):
        for j in range(len(network.edgeid_list[i])):
...
>>>>>>> ChengLi
    ...
    def getWeighedShortestPaths(self, edgeID_from, capacity, currentEdgePerson):
        ...
        cost[start_id][0] = 0
        personNum = [0 for i in range(self._numNodes)]
        personNum[start_id] = len(currentEdgePerson.get(self.edgeid_list[self._nodeDict[self.getEdge(edgeID_from).getFromNode().getID()]][start_id],[]))
        pre_id = [-1 for i in range(self._numNodes)]
        pre_id[start_id] = self._nodeDict[self.getEdge(edgeID_from).getFromNode().getID()]
        origin = self._nodeDict[self.getEdge(edgeID_from).getFromNode().getID()]
        pre_id[origin] = origin
        ...
        while queue:
            ...
            for neighbor in self.adjacent_list[pop_id]:
                ...
                if (currentCost < cost[neighbor][0] or cost[neighbor][0] == -1):
                    cost[neighbor][0] = currentCost
                    pre_id[neighbor] = pop_id
                    currentPersonNum = personNum[pop_id] + len(currentEdgePerson.get(self.edgeid_list[pop_id][neighbor], []))
                    if (currentPersonNum >= capacity):
                        endID = neighbor
                        FindTarget = True
                        break
